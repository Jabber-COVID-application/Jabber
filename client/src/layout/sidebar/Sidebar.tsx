...
const Sidebar = observer((props: Props): JSX.Element => {
    ...
    <aside className={styles.sidebar}>
      ...
      <div className={styles.toolbar}>
        <div className={styles.upper}>
          {sidebar.map((item) => <ToolbarItem key={item.text} {...item} type="icon" />)}
        </div>

        <div className={styles.lower}>
          <ToolbarItem icon={UserIcon} text={user.fullName} type="avatar" selected={true} />
        </div>
      </div>
    </aside>
  );
